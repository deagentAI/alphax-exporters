...
type BSCEventListenerJob struct {
	startBlock        uint64
	endBlock          uint64
...
func NewBSCEventListenerJob(ci *contracts.Instance) *BSCEventListenerJob {
	ct, err := alphax.NewAlphax(ci.Address, ci.Client)
	if err != nil {
		panic(err)
	}
	return &BSCEventListenerJob{
		startBlock:       0,
		endBlock:         0,
		contractInstance: ci,
		alphaxContract:   ct,
...
func (e *BSCEventListenerJob) Do(ctx context.Context) {
	hlog.CtxInfof(ctx, "[BSCEventListenerJob] start")
	var err error
	...
	if err != nil {
		hlog.CtxErrorf(ctx, "[BSCEventListenerJob] chain name: %v, get block number failed, err: %v", chainName, err)
	}
	if e.startBlock >= e.endBlock {
		return
	}
	hlog.CtxInfof(ctx, "[BSCEventListenerJob] chain name: %v, start listen on block: %v, end block: %v", chainName, e.startBlock, e.endBlock)
	...
	hlog.CtxInfof(ctx, "[BSCEventListenerJob] chain name: %v, update last listen on block number: %d", chainName, e.startBlock)
	_, _ = redis.AlphaXClient.Set(ctx, lastListenOnBlockNumber, e.startBlock, 0).Result()
}

func (e *BSCEventListenerJob) processCheckinEvent(ctx context.Context, opts *bind.FilterOpts) {
	chainName := e.contractInstance.Name

	eventIterator, err := e.alphaxContract.FilterCheckinEvent(opts, nil)
	if err != nil {
		hlog.CtxErrorf(ctx, "[BSCEventListenerJob] chain name: %v, get checkin event error: %v", chainName, err)
		return
	}
...
func (e *BSCEventListenerJob) processSignalPredictionEvent(ctx context.Context, opts *bind.FilterOpts) {
	chainName := e.contractInstance.Name

	eventIterator, err := e.alphaxContract.FilterSignalPredictionEvent(opts, nil)
	if err != nil {
		hlog.CtxErrorf(ctx, "[BSCEventListenerJob] chain name: %v, get signal prediction event error: %v", chainName, err)
		return
	}
...
func (e *BSCEventListenerJob) Interval() time.Duration {
	return 1 * time.Second
}

func (e *BSCEventListenerJob) DisableSerializable() bool {
	return false
}
